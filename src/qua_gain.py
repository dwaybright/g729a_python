...
def Qua_gain(
    ...
    gcode0, exp_gcode0 = Gain_predict(past_qua_en, code, L_subfr)
    ...
    cand1, cand2 = Gbk_presel(best_gain, cand1, cand2, gcode0_org)
    ...
    for i in range(0, 5):
        ...
        # L_tmp:Q[exp_g_coeff[i]+16-j]
        L_tmp = L_shr(L_tmp, j)
        coeff[i], coeff_lsf[i] = L_Extract(L_tmp)  # DPF
    ...
    if tameflag == 1:
        for i in range(0, NCAN1):
            for j in range(0, NCAN2):
                ...
                if g_pitch < GP0999:
                    ...
                    L_tmp = L_add(L_tmp, Mpy_32_16(
                        coeff[1], coeff_lsf[1], g_pitch))
                    L_tmp = L_add(L_tmp, Mpy_32_16(
                        coeff[2], coeff_lsf[2], g2_code))
                    L_tmp = L_add(L_tmp, Mpy_32_16(
                        coeff[3], coeff_lsf[3], g_code))
                    L_tmp = L_add(L_tmp, Mpy_32_16(
                        coeff[4], coeff_lsf[4], g_pit_cod))
    ...
    result = add(map1[index1] * NCODE2, map2[index2])
    return (result, gain_pit, gain_cod)
